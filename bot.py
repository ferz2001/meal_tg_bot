            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Увеличивай калорийность на 50-100ккал, если она слишком низкая. "
                        "Если на фото нет блюда, но в дополнении указаны детали (например, 'кофе с молоком 300мл'), используй информацию из дополнения для формирования ответа. "
                        "Если на фото нет явного блюда, но описание в дополнении даёт чёткие указания, используй его в первую очередь, даже если на фото только стакан или неопределённый объект. "
                    ...
                    "content": [
                        {
                            ...
                            "text": (
                                ...
                                "Если на фото нет блюда или информация о весе/составе неполная, используй данные, предоставленные пользователем в виде дополнения. "
                                "Преимущественно возвращай информацию из дополнения, если она чётко указывает на блюдо, даже если на фото этого блюда нет. "
                                "Если на фото нет блюда, и по дополнительной информации невозможно сформировать ответ, верни строго: 'Блюдо не найдено'."
                            )
                        },
                        {
                            "type": "text",
                            "text": f"Дополнение: {' '.join(message.caption.split()[1:])}. "
                        },
                        {
                            "type": "image_url",
                            "image_url": {
